...
public class Copper(WorldServer server, Mundane mundane) : MundaneScript(server, mundane)
{
    ...
    public override void OnResponse(WorldClient client, ushort responseID, string args)
    {
        ...
        switch (responseID)
        {
            case 1:
                {
                    ...
                    if (client.Aisling.ExpLevel >= 15)
                        options.Add(new Dialog.OptionsDataItem(0x05, "{=bRefine"));
                    else
                    {
                        client.SendOptionsDialog(Mundane, "This ore is too high level for you. (Insight: 15)");
                        return;
                    }
            ...
            case 5:
                if (RefineNode(client.Aisling) && client.Aisling.ExpLevel >= 15)
                {
                    ...
                    client.GiveExp(5000);
                    client.SendServerMessage(ServerMessageType.ActiveMessage, "Refining success! 5,000 exp");
                    client.CloseDialog();
                }
                else
                {
