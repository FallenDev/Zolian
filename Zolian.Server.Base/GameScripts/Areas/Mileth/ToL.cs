...
public class ToL : AreaScript
{
    ...
    private void HandleMapAnimations(TimeSpan elapsedTime)
    {
        ...
        if (a)
        {
            _playersOnMap.Values.FirstOrDefault()?.SendTargetedClientMethod(PlayerScope.NearbyAislings, client => client.SendAnimation(192, new Position(15, 55)));
            _playersOnMap.Values.FirstOrDefault()?.SendTargetedClientMethod(PlayerScope.NearbyAislings, client => client.SendAnimation(192, new Position(20, 55)));
            _playersOnMap.Values.FirstOrDefault()?.SendTargetedClientMethod(PlayerScope.NearbyAislings, client => client.SendAnimation(192, new Position(21, 40)));
            _playersOnMap.Values.FirstOrDefault()?.SendTargetedClientMethod(PlayerScope.NearbyAislings, client => client.SendAnimation(192, new Position(14, 40)));
        }

        if (b)
        {
            _playersOnMap.Values.FirstOrDefault()?.SendTargetedClientMethod(PlayerScope.NearbyAislings, client => client.SendAnimation(96, new Position(17, 59)));
            _playersOnMap.Values.FirstOrDefault()?.SendTargetedClientMethod(PlayerScope.NearbyAislings, client => client.SendAnimation(96, new Position(18, 59)));
        }
    }
}