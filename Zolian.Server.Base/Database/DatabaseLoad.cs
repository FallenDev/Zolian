...
public abstract class DatabaseLoad
{
    ...
    private static void WorldWarps(string conn)
    {
        var count = 0;
        ...
        {
            var sConn = new SqlConnection(conn);
            const string sql = "SELECT * FROM ZolianMaps.dbo.WorldWarps";
        ...
        ServerSetup.EventsLogger($"World Warp Templates: {count}");
        Warps(ZolianMapsConn, count);
    }

    private static void Warps(string conn, int count)
    {
        try
        {
            var sConn = new SqlConnection(conn);
    ...
    private static void Monsters(string conn)
    {
        try
        {
            ...
            [
                ...
                "Summoned", "FilthForest", "LouresSewers", "Chaos"
            ];
    ...
    private static void Mundanes(string conn)
    {
        try
        {
            ...
            [
                ...
                "Lynith", "CthonicRemains", "Gobleregan", "Evermore"
            ];
